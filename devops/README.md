...
DEPLOY_ENV=prod
DEPLOY_HOST=volto-footer-project.example.com
DEPLOY_PORT=22
DEPLOY_USER=plone
...
You need either a Ubuntu or Debian based system for volto-footer-project.example.com, enable SSH, and install a supported version of Python 3 on that system.
...
prod:
  hosts:
    volto-footer-project.example.com:
      ansible_user: root
      host: volto-footer-project
      hostname: volto-footer-project.example.com
```
...
Deploy the stack defined in `devops/stacks/volto-footer-project.example.com.yml` to the remote server with:
